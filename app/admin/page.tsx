        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-lms-border bg-lms-bg">
              ...
              <th className="text-left p-3 font-medium text-lms-text-secondary hidden lg:table-cell">Modules</th>
          ...
          <tbody>
            {courses.map((c: any) => (
              <tr key={c.id} className="border-b border-lms-border hover:bg-lms-bg/50 transition-colors">
                ...
                <td className="p-3 text-lms-text-secondary hidden md:table-cell">{c.archetype || "custom"}</td>
                <td className="p-3 hidden sm:table-cell">
                  <span className={`lms-badge ${c.status === "published" ? "lms-badge-success" : "lms-badge-warning"}`}>
                    {c.status}
                  </span>
                </td>
                <td className="p-3 text-lms-text-secondary hidden lg:table-cell">{c.moduleCount ?? "—"}</td>
