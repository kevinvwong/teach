 ...
 * quiz-renderer.js
 *
 * Client-side IRT quiz component for vowel teams literacy course.
 * See civil_war/assessments/quiz-renderer.js for full specification.
 *
 * Usage:
 *   <div id="quiz-container" data-item-bank="../assessments/item-bank.json"
 *        data-domain="vowel-teams-module-2" data-n-items="5" data-se-threshold="0.5">
 *   </div>
...
(function () {
  ...
  async function handleResponse(item) {
    ...
    state.responses.push({ itemId: item.id, response, correct, responseTimeMs: 0 });
  ...
  function showResults() {
    ...
    const bands = [
      { max: -1.5, label: 'Beginning', desc: 'Great start! Keep practicing these vowel teams.' },
      { max: -0.5, label: 'Emerging', desc: 'You\'re learning the patterns. Review the ones you missed.' },
      { max: 0.5, label: 'Developing', desc: 'Good progress! You know most of these vowel teams.' },
      { max: 1.5, label: 'Proficient', desc: 'Strong understanding! You can read these patterns confidently.' },
      { max: Infinity, label: 'Advanced', desc: 'Excellent! You\'ve mastered these vowel teams.' },
    ];
    ...
    feedbackEl.innerHTML = `
      <div class="info-box">
        ...
        <p><strong>Ability estimate:</strong> ${state.theta.toFixed(2)}</p>
